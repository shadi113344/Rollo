...
function normalizeMeta(entry) {
  ...
  if (typeof entry?.durationSec === "number" && entry.durationSec > 0) {
    meta.durationSec = entry.durationSec;
  }
  if (typeof entry?.width === "number" && entry.width > 0) meta.width = entry.width;
  if (typeof entry?.height === "number" && entry.height > 0) meta.height = entry.height;
  return meta;
}
...
function createMetadataStore(videosDir, legacyPath) {
  ...
  function migrateFromLegacy(groupIds) {
    ...
    for (const key of keys) {
      ...
      if (writeGroupFile(groupId, data)) migrated++;
    }
  ...
  function moveVideoMeta(fromGroup, toGroup, filename, destFilename) {
    const fromData = readGroupFile(fromGroup);
    const meta = fromData[filename] ? normalizeMeta(fromData[filename]) : null;
    if (fromData[filename]) {
      delete fromData[filename];
      writeGroupFile(fromGroup, fromData);
    }
    if (!meta) return;
    const toData = readGroupFile(toGroup);
    toData[destFilename] = meta;
    writeGroupFile(toGroup, toData);
  }
