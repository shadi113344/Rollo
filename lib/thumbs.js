...
const { isImageFile, isTextFile, isHtmlFile, isPdfFile } = require("./media");
...
function createThumbService(videosDir, getFfmpegCommand) {
  ...
  async function ensureThumb(groupId, filename) {
    ...
    if (isImageFile(filename) || isTextFile(filename) || isHtmlFile(filename) || isPdfFile(filename)) return null;
