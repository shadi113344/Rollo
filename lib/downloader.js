...
function buildDownloadAttempts({ url, outputDir, quality, xAuth, ffmpegDir }) {
  ...
  if (isTwitterUrl(url)) {
    ...
    attempts.push(buildYtdlpArgs({ url, outputDir, quality, xAuth, ffmpegDir }));
  }
...
function createDownloader({ onComplete, dataDir } = {}) {
  ...
  function getInfo() {
    ...
    if (ytdlpCommand && !ffmpegCommand) {
      tips.push(
        "ffmpeg not found — install with: winget install Gyan.FFmpeg, then restart Rollo"
      );
    }
    if (!xConnected) {
  ...
  async function startDownload({ url, groupId, outputDir, quality = "fast" }) {
    ...
    if (!available) {
      throw new Error("yt-dlp is not installed on this server. Install it with: winget install yt-dlp");
    }
