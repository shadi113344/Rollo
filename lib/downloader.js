...
function buildProbeArgs(url, xAuth) {
  ...
  if (isTwitterUrl(url) && !hasXAuth(xAuth)) {
    args.push("--extractor-args", "twitter:api=syndication");
  }
  args.push(url);
  return args;
...
function createDownloader({ onComplete, dataDir } = {}) {
  ...
  async function listFormats(url) {
    ...
    if (usesAndroidBridge()) {
      const stdout = await probeFormatsAndroid(trimmed, dataDir, xAuth?.cookiesFile || "");
      const data = JSON.parse(stdout);
      return parseFormatsFromInfo(data);
    }

    const args = buildProbeArgs(trimmed, xAuth);
